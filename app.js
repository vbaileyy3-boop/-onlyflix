/* ============================================================
   app.js — OnlyFlix UI (TMDB live) + multi-format player
   ============================================================ */
...
async function initHero(){
  const t = await api.trendingAll();
  heroItems = t.filter(x=>x.backdrop && x.overview).slice(0,6);
...
/* ---------- TRENDING board ---------- */
const PERIOD = {
  "24h": {movie:()=>tmdb("/trending/movie/day").then(d=>normList(d.results)), series:()=>tmdb("/trending/tv/day").then(d=>normList(d.results))},
...
function resetGrid(){gridState.page=1;gridState.items=[];$("#grid").innerHTML='<div class="boot">Loading…</div>';loadGridPage(true);}
...
async function renderSearch(q){
  $("#hero").style.display="none"; $("#infoBlocks").classList.remove("show");
  $("#view").innerHTML=`<div class="page-head"><h1>Search: “${q}”</h1></div><div class="grid" id="sgrid"><div class="boot">Searching…</div></div>`;
  const items=await api.search(q);indexItems(items);
  $("#sgrid").innerHTML=items.length?items.map(cardHTML).join(""):`<div class="empty">No results for “${q}”.</div>`;
}

/* ---------- ROUTER ---------- */
function route(name){
  if(name!=="home") clearInterval(heroTimer); // Clear auto hero timer on other pages
...
function closeDetail(){$("#detailModal").classList.remove("open");if(!$("#playerModal").classList.contains("open"))document.body.style.overflow="";}
...
function openPlayer(id, season, episode){
  ...
  // Generate trackId configuration for memory tracking (per episode if TV, per ID if movie)
  ...
  let defIdx = 0;
  if (FRAMED) {
    const demoIdx = sources.findIndex(s => s.type !== "embed");
...
function loadSource(src){
  ...
  // RESUME PLAYBACK ACCELERATION INTERCEPT
  v.onloadedmetadata = () => {
...
function bindPlayer(){
  ...
  v.ontimeupdate=()=>{
    ...
    // Write runtime stamp positions continuously every second
  ...
  v.onended=()=>{
    $("#btnPlay").textContent="▶";
    // Wipe layout position once completed so users aren't trapped on credits loop next time
    if (PLAYER_CTX && PLAYER_CTX.trackId) {
      clearProgress(PLAYER_CTX.trackId);
    }
  };
...
(async function init(){
  ...
  try{
    await loadGenres();
    await Promise.all([initHero(), renderHome()]);
  }catch(e){

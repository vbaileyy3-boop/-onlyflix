/* ============================================================
   config.js — TMDB config + STREAM SOURCE RESOLVER LAYER (FIXED v8)
   ============================================================ */
...
/* ------------------------------------------------------------
   SOURCE_REGISTRY — episode-specific & show-level overrides
   Keys: "tv:{tmdbId}:S{season}E{episode}" or "tv:{tmdbId}" or "movie:{tmdbId}"
   ------------------------------------------------------------ */
...
function resolveSources(item, season, episode) {
  ...
  EMBED_SERVERS.forEach(srv => {
    out.push({
      label: srv.name,
      type: "embed",
      url: isTV ? srv.tv(id, s, e) : srv.movie(id),
    });
  });

  // 4) Deterministic demo pool
  const pool = DEMO_POOL[Math.abs(parseInt(id, 10) || 0) % DEMO_POOL.length];
  pool.variants.forEach(v => {
    out.push({
      label: `Demo · ${pool.name}${v.type === "hls" ? " (HLS)" : ""}`,
      type: v.type,
      url: v.url,
    });
  });
  ...
  const seen = new Set();
  out = out.filter(s => s.url && !seen.has(s.url) && seen.add(s.url));
